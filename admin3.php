...
$data = file_get_contents('data3.json');
if ($data !== false) {
...
if (isset($_POST['delete'])) {
    ...
    if (!empty($eventIds)) {
        // Filter the events array to exclude the selected events
        ...
        file_put_contents('data3.json', $jsonData);
...
if (!empty($search)) {
    $events = array_filter($events, function ($event) use ($search) {
        return strpos($event['title'], $search) !== false;
    });
}
...
<nav class="navbar navbar-inverse navbar-fixed-top">
    <div class="container">
        <div class="navbar-header">
            ...
            <a class="navbar-brand" href="admin3.php">
        ...
        <div class="collapse navbar-collapse">
            <ul class="nav navbar-nav navbar-right">
                ...
                <li><a href="admin3.php?logout=1">Logout</a></li>
            </ul>
        </div>
    </div>
</nav>
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" id="deleteForm">
            <table class="table table-hover">
                ...
                <?php foreach ($events as $event): ?>
                    <tr>
                        <td><input type="checkbox" name="event[]" value="<?php echo $event['id']; ?>"/></td>
                        <td class="text-center event-id"><?php echo $event['id']; ?></td>
                        <td width="40%">
                            <a href="#" class="content-link" data-toggle="modal"
                               data-target="#contentModal_<?php echo $event['id']; ?>">
                                <?php echo $event['title']; ?>
                            </a>
                            <!-- Content Modal -->
                            <div class="modal" id="contentModal_<?php echo $event['id']; ?>" tabindex="-1"
                                 role="dialog" aria-labelledby="contentModalLabel_<?php echo $event['id']; ?>">
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title"
                                                id="contentModalLabel_<?php echo $event['id']; ?>"><?php echo $event['title']; ?></h4>
                                        </div>
                                        <div class="modal-body">
                                            <ul class="list-group">
                                                <li class="list-group-item"><strong>Date:</strong> <?php echo $event['date']; ?></li>
                                                <li class="list-group-item"><strong>Host:</strong> <?php echo $event['host']; ?></li>
                                                ...
                                                <li class="list-group-item"><strong>Link:</strong> <a href="<?php echo $event['link']; ?>"><?php echo $event['link']; ?></a></li>
                                            </ul>
                                        </div>
                        ...
                        <td class="text-center" width="15%"><?php echo isset($event['date']) ? $event['date'] : ''; ?></td>
                        <td class="text-center" width="15%"><?php echo isset($event['host']) ? $event['host'] : ''; ?></td>
                        <td class="text-center" width="15%"><?php echo isset($event['location']) ? $event['location'] : ''; ?></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
